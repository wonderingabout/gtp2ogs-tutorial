...
note : in that example some commands like `--maxactivegames` , `--maxtotalgames` , `--komi` exist only on the devel branch, so i cant stress more the importance of upgrading to devel if you didnt already do it
